layui.define('jquery', function (exports) {
    ...
    var CodeMirror = (function () {
            ...
            function onPaste(e) {
                ...
                if (e.clipboardData.types == "text/plain") {
                    return true;
            ...
            function imgReader(item) {
                if (item.kind == "file" && item.type == "image/png") {
                    ...
                    reader.onload = function (e) {
                        ...
                        // 把图片放在网页最下面，以便预览
                        $.post(".php", {'urls': e.target.result}, function (data) {
                ...
                var ELEMENTS = [
                    ...
                    },
                    {
                        patterns: 'code', replacement: function (str, attrs, innerHTML) {
                            return innerHTML ? '`' + innerHTML.replace(/<.*?>/ig, '') + '`' : '';
                        }
                    },
                    // 下面这部分是我自己写的。
                    {
                        patterns: 'table', replacement: function (str, attrs, innerHTML) {
                            innerHTML = innerHTML.replace(/<\/tr>/ig, '\r\n').replace(/<tr(\s[^>]+)?>/ig, '|').replace(/<\/t[hd]>/ig, '|').replace(/<t[hd](\s[^>]+)?>/ig, '').replace(/<\/?tbody[^>]*?>/ig, '').replace(/<thead(\s[^>]+)?>([\s\S]*?)\n<\/thead>/ig, function (all, a, b) {
                    ...
                    {
                        patterns: ['font', 'span'], replacement: function (str, attrs, innerHTML) {
                            return innerHTML;
                        }
                    },
                    // 上面的内容是我自己写的
                    {
                        patterns: 'pre', replacement: function (str, attrs, innerHTML) {
                            return innerHTML ? '```\n' + innerHTML + '\n```\n' : ''
                        }
                    },
                    {
